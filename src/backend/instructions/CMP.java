...
static int operand_2;
    ...
    public void execute(String ins, String token3, String token4) {
       if(ScanFile.a_mode==1 && token4.contains("#")){
    	   execute(ins,token3,token4,0);
    	   return;
       }
       else if(ScanFile.a_mode==1 && backend.Register.convertRegister(token4)<=7 && backend.Register.convertRegister(token3)<=7){
    	   execute(ins,token3,token4,0,0);
    	   return;
       }
       else if(ScanFile.a_mode==1){
    	   execute(ins,token3,token4,0,0,0);
    	   return;
       }
        ...
        binaryconvert[22]=1;
        binaryconvert[21]=0;
          backend.InstructionSet.CMP token1=backend.InstructionSet.CMP.valueOf(ins);
               switch(token1)
               {
                   case CMP:
                       binaryconvert[31]=1;
                        ...
                        execCMP(token3,token4);
                         if(chck==1 && FrontEnd.build_flag==0)
                                backend.Condition.updateFlags(b, no2, no3, chck,1);
                          else if(FrontEnd.build_flag==0)
                                backend.Condition.updateFlags(b, no2, (int)operand_2, chck,1);
                        break;
                   case CMPEQ:
                       binaryconvert[31]=0;
                        binaryconvert[30]=0;
    ...
    public void execute(String ins, String token3, String token4,int x,int y,int z){
        ...
        backend.InstructionSetThumb.CMP token1=backend.InstructionSetThumb.CMP.valueOf(ins);
    
        switch(token1){
        case CMP:
        	execCMP(token3,token4,x,y,z);
        	if(FrontEnd.build_flag==0)
        	if(chck==1)
                backend.Condition.updateFlags(b, no2, no3, chck,1);
          else
    ...
    public static void execCMP(String token3, String token4,int x,int y,int z){
    ...
    public void execute(String ins, String token3, String token4,int x,int y){
    	binaryconvert16=new int[16];
        binaryconvert16[15]=0;
        binaryconvert16[14]=1;
        binaryconvert16[13]=0;
    ...
    public static void execCMP(String token3, String token4,int x,int y){
   ...
   frontend.FrontEnd.statuswindow.setCaretPosition(frontend.FrontEnd.statuswindow.getText().length()); }
    ...
    public static void execCMP(String token3, String token4,int x){
...
public static void execCMP(String token3, String token4)
    {
        ...
        frontend.FrontEnd.statuswindow.setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());  }
               ...
               else if(no2==-2)
               {
            	   if(handlers.cmd_var==1){
                    ...
                    frontend.FrontEnd.statuswindow.append("\nERROR in line "+backend.ScanFile.curent_line+":"+"The operands of CMP should be a register and "+token3+" is not a valid register.");
        frontend.FrontEnd.statuswindow.setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());} frontend.FrontEnd.exceptionraised++;       return;}    /*      try {
