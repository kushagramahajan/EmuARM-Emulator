...
public class LSR implements InsInterface4,InsInterface3{
 ...
 public void execute(String ins, String token2, String token3, String token4){
		...
		backend.InstructionSetThumb.LSR token1=backend.InstructionSetThumb.LSR.valueOf(ins);
 	...
 	try{
 		...
 		operand_2=backend.BarrelShifter.lsrShift(operand_1, token4);
    	...
    	if(FrontEnd.build_flag==0){
 		...
 		if(handlers.cmd_var==1){
      	  System.out.println("\nERROR in line "+backend.ScanFile.curent_line+":"+"LSR operation failed");
        }
        else{
...
frontend.FrontEnd.statuswindow.setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());}
	...
	public void execute(String ins, String token2, String token3){
 	...
 	try{
 		...
 		operand_2=backend.BarrelShifter.lsrShift(operand_1, token3);
 		if(FrontEnd.build_flag==0){
     	backend.Register.r[no].storeValue(operand_2);
     	 backend.Condition.updateFlags(no);
 		}
