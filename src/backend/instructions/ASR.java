...
public class ASR implements InsInterface4,InsInterface3{
 ...
 public void execute(String ins, String token2, String token3, String token4){
		...
		backend.InstructionSetThumb.ASR token1=backend.InstructionSetThumb.ASR.valueOf(ins);
 	...
 	try{
 		...
 		operand_2=backend.BarrelShifter.asrShift(operand_1, token4);
 		...
 		if(handlers.cmd_var==1){
        	  System.out.println("\nERROR in line "+backend.ScanFile.curent_line+":"+"ASR operation failed");
          }
          else{
...
frontend.FrontEnd.statuswindow.setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());}
	...
	public void execute(String ins, String token2, String token3){
 	...
 	try{
 		...
 		operand_2=backend.BarrelShifter.asrShift(operand_1, token3);
